    ...
    <div class="border-t border-gray-200">
        <dl>
            ...
            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                    @if(is_array($giaoVien->bang_cap) && count($giaoVien->bang_cap) > 0)
                        <ul class="list-disc pl-5">
                            @foreach($giaoVien->bang_cap as $bangCap)
                                <li>{{ $bangCap }}</li>
                            @endforeach
                        </ul>
                    @elseif(is_string($giaoVien->bang_cap) && !empty($giaoVien->bang_cap))
                        <div>{{ $giaoVien->bang_cap }}</div>
                    @else
                        Chưa cập nhật
                    @endif
                </dd>
