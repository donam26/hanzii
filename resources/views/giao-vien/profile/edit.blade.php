...
<div class="bg-white shadow overflow-hidden sm:rounded-lg">
    ...
    <div class="border-t border-gray-200">
        <form action="{{ route('giao-vien.profile.update') }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                ...
                <div class="sm:col-span-6">
                    ...
                    <div class="mt-1">
                        <textarea name="bang_cap" id="bang_cap" rows="3" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">{{ old('bang_cap', $giaoVien->bang_cap) }}</textarea>
                    </div>
                    @error('bang_cap')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
