...
@section('page-heading', 'Tạo thông báo mới')
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow rounded-lg">
                <div class="px-4 py-5 sm:p-6">
                    <form action="{{ route('giao-vien.thong-bao.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <select id="lop_hoc_id" name="lop_hoc_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                                    ...
                                    @foreach($lopHocs as $lopHoc)
                                        <option value="{{ $lopHoc->id }}" @if(old('lop_hoc_id') == $lopHoc->id) selected @endif>
                            ...
                            <div>
                                ...
                                <input type="text" name="tieu_de" id="tieu_de" value="{{ old('tieu_de') }}" class="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md">
                                @error('tieu_de')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea id="noi_dung" name="noi_dung" rows="8" class="tinymce shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md">{{ old('noi_dung') }}</textarea>
                                @error('noi_dung')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">File đính kèm (nếu cần)</label>
