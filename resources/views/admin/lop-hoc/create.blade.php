...
@section('page-heading', 'Tạo lớp học mới')
...
@section('content')
    <div class="mb-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            <h2 class="text-xl font-semibold text-gray-800">Tạo lớp học mới</h2>
        ...
        <form action="{{ route('admin.lop-hoc.store') }}" method="POST">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="ten" id="ten" value="{{ old('ten') }}" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm" placeholder="Nhập tên lớp học">
                            @error('ten')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="ma_lop" id="ma_lop" value="{{ old('ma_lop') }}" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm" placeholder="Nhập mã lớp học (VD: JAVA-01)">
                            @error('ma_lop')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="khoa_hoc_id" name="khoa_hoc_id" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm">
                                ...
                                @foreach($khoaHocs as $id => $ten)
                                    <option value="{{ $id }}" {{ old('khoa_hoc_id', ($selectedKhoaHoc ? $selectedKhoaHoc->id : null)) == $id ? 'selected' : '' }}>{{ $ten }}</option>
                                @endforeach
                            </select>
                            @error('khoa_hoc_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="so_luong_toi_da" id="so_luong_toi_da" value="{{ old('so_luong_toi_da', 20) }}" required min="1" max="100" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="trang_thai" name="trang_thai" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm">
                                <option value="sap_khai_giang" {{ old('trang_thai') == 'sap_khai_giang' ? 'selected' : '' }}>Sắp khai giảng</option>
                                <option value="dang_dien_ra" {{ old('trang_thai') == 'dang_dien_ra' ? 'selected' : '' }}>Đang diễn ra</option>
                    ...
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="hinh_thuc_hoc" name="hinh_thuc_hoc" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm">
                                <option value="online" {{ old('hinh_thuc_hoc') == 'online' ? 'selected' : '' }}>Trực tuyến</option>
                                <option value="offline" {{ old('hinh_thuc_hoc') == 'offline' ? 'selected' : '' }}>Tại trung tâm</option>
                            </select>
                            @error('hinh_thuc_hoc')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="lich_hoc" id="lich_hoc" value="{{ old('lich_hoc') }}" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm" placeholder="VD: Thứ 2, 4, 6 - 18h30-21h00">
                            @error('lich_hoc')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="ngay_bat_dau" id="ngay_bat_dau" value="{{ old('ngay_bat_dau', date('Y-m-d')) }}" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm">
                            @error('ngay_bat_dau')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="ngay_ket_thuc" id="ngay_ket_thuc" value="{{ old('ngay_ket_thuc', date('Y-m-d', strtotime('+3 months'))) }}" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm">
                            @error('ngay_ket_thuc')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="giao_vien_id" name="giao_vien_id" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm">
                                ...
                                @foreach($giaoViens as $id => $ten)
                                    <option value="{{ $id }}" {{ old('giao_vien_id') == $id ? 'selected' : '' }}>{{ $ten }}</option>
                                @endforeach
                            </select>
                            @error('giao_vien_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="tro_giang_id" name="tro_giang_id" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm">
                                ...
                                @foreach($troGiangs as $id => $ten)
                                    <option value="{{ $id }}" {{ old('tro_giang_id') == $id ? 'selected' : '' }}>{{ $ten }}</option>
                                @endforeach
                            </select>
                            @error('tro_giang_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="mt-4">
                    ...
                    <textarea name="ghi_chu" id="ghi_chu" rows="3" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm" placeholder="Thông tin bổ sung về lớp học...">{{ old('ghi_chu') }}</textarea>
                </div>
            </div>
            
            <div class="px-6 py-4 bg-gray-50 text-right">
                <a href="{{ route('admin.lop-hoc.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                    Hủy
                </a>
